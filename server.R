...
library(here) ; library(rdrop2) ; library(shiny) ; library(DT) # chargement des packages
...
server <- function(input, output, session) {
  ...
  output$render_recherche_nom <- renderUI({ # recherche par nom / mots-cles
  ...
  output$render_recherche_carte <- renderUI({ # recherche plus precise par type de carte
  ...
  output$render_recherche_sig <- renderUI({# recherche plus precise par disponibilite de SIG
    checkboxInput("rech_sig", "SIG", FALSE)
  })
  
  output$render_recherche_brutes <- renderUI({# recherche plus precise par disponibilite de donnees brutes
  ...
  output$texte_recherche_nulle <- renderUI({
    ...
    if(aucrech == 0){fluidRow(style = "padding:30px",
                              fluidRow("Aucune recherche saisie"), br(),
                              fluidRow(style="color:#009193", tags$em("Voir l'onglet"), tags$b("Informations"), tags$em("pour avoir des conseils sur la recherche de données,
                                                                                                et pour le fonctionnement du site en général")))}
    else if(aucrech == 1 & length(numfin) == 0){fluidRow(style = "padding:30px",
                                                         fluidRow("Aucune donnée ne satisfait tous les critères"), br(),
                                                         fluidRow(style="color:#009193", tags$em("Enlever ou modifier un champ pour avoir plus de chance de trouver une donnée")),
                                                         fluidRow(style="color:#009193", tags$em("Voir l'onglet"), tags$b("Informations"), tags$em("pour avoir des conseils sur la recherche de données")))}
  })
    ...
    if(length(numfin)>0){#s'il y a une (et une seule) donnee a afficher
      table_info <- datatable(cbind(t(ress[numfin,c(1:9,12:14)]), rep("",length(c(1:9,12:14)))), # informations + colonne vide (qui sera remplie avec les liens)
                              ...
                              escape = F, options = list(sDom  = "<'top'>t<'bottom'>", ordering = F,  # options pour l'affichage du tableau, notamment pour qu'il soit simple
                                                         pageLength = 15,lengthMenu = c(15, 30),columnDefs = list(list(width = "900px", targets = 1)))) %>% 
  ...
  output$render_liste_categorie <- renderUI({
    selectInput("liste_categorie", "Liste de données par catégorie", choices = c("","Toutes",sort(unique(ress$Catégorie))))
  })
  
  output$table_categorie_choisie <- renderDataTable({ # tableau des donnees de la categorie choisie
    if(input$liste_categorie =="Toutes"){tableListe1 <- cbind(sort(ress[,c(1)])) # toutes les categories
  ...
  output$render_liste_theme <- renderUI({
  ...
  output$render_liste_themecovadis <- renderUI({
  ...
  output$render_liste_organisme <- renderUI({
    selectInput("liste_organisme", "Liste de données par organisme", choices = c("","Tous",sort(unique(ress$Organisme))))
  })
  
  output$table_organisme_choisi <- renderDataTable({ # tableau des donnees du theme covadis choisi
    if(input$liste_organisme =="Tous"){tableListe4 <- cbind(sort(ress[,c(1)])) # tous les themes covadis
    colnames(tableListe4) <- "Nom de la donnée"
    return(tableListe4)}
    else if(input$liste_organisme !=""){tableListe4 <- cbind(sort(ress[which(ress$Organisme %in% input$liste_organisme),c(1)])) # seulement les themes covadis choisis
    colnames(tableListe4) <- "Nom de la donnée"
    return(tableListe4)}
  }, escape = F, options = list(sDom  = "<'top'>t<'bottom'>", ordering = F,pageLength = nlength,lengthMenu = c(nlength, 2*nlength)), rownames = F)
  
  output$render_liste_motcle <- renderUI({
    selectInput("liste_motcle", "Liste de données par mots-clés", choices = c("","Tous",sort(unique(unlist(strsplit(ress$`Mots-clés`, ", "))))))
  })
  
  output$table_motcle_choisi <- renderDataTable({ # tableau des donnees du theme covadis choisi
    if(input$liste_motcle =="Tous"){tableListe5 <- cbind(sort(ress[,c(1)])) # tous les themes covadis
    colnames(tableListe5) <- "Nom de la donnée"
    return(tableListe5)}
    else if(input$liste_motcle !=""){
      tableListe5 <- cbind(sort(ress[which(sapply(strsplit(ress$`Mots-clés`, ", "), function(y) input$liste_motcle %in% y)),
                                     c(1)])) # seulement les themes covadis choisis
      colnames(tableListe5) <- "Nom de la donnée"
      return(tableListe5)}
  }, escape = F, options = list(sDom  = "<'top'>t<'bottom'>", ordering = F,pageLength = nlength,lengthMenu = c(nlength, 2*nlength)), rownames = F)
  ...
  output$render_modif_donnee <- renderUI({# nom de la donnee a modifier
    selectizeInput("donnee_modif", label = "Donnée à modifier", choices = sort(c("",ress$Nom)), multiple = F, width = 550)
  })
  
  output$render_modif_champ <- renderUI({# nom de l'information a modifier (adresse, organisme, ...)
  ...
  output$render_suppr_donnee <- renderUI({
